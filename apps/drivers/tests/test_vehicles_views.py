...
class VehiclesCreationApiViewTestCase(ViewVehicleTestCase):
    def test_create_vehicle(self):
        ...
        payload = {"plate_number": "1234", "model": "asas", "year": 1234, "color": "blue"}
        response = self.client.post(url, payload, headers={"Authorization": self.authorization})
...
class VehiclesDetailApiViewTestCase(ViewVehicleTestCase):
    ...
    def test_get_vehicle_not_found(self):
        url = reverse_lazy('drivers:vehicles_detail', kwargs={"uuid": uuid4().__str__()})
        response = self.client.get(url, headers={"Authorization": self.authorization})
        
        self.assertEqual(response.status_code, 404)
        self.assertEqual(response.data["detail"], VehicleDoesNotExistsException.default_detail)
...
class VehiclesDeleteApiViewApiView(ViewVehicleTestCase):
    ...
    def test_delete_vehicles(self):
        ...
        response = self.client.delete(url, headers={"Authorization": self.authorization})
        
        self.assertEqual(response.status_code, 204)
        self.assertFalse(Vehicles.objects.filter(id=self.vehicle.id).exists())
    
    def test_delete_vehicle_not_found(self):
        url = reverse_lazy('drivers:vehicles_delete', kwargs={"uuid": uuid4().__str__()})
