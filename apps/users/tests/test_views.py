...
class TestUserRegisterView(TestCase):
    ...
    def test_user_creation(self):
        ...
        res = self.factory.post(reverse_lazy("users:user_register"), data=payload, format="json")
...
class TestUserVerifyView(TestCase):
    ...
    def test_verify_user(self):
        token = create_jwt_token({"type":"_email_confirmation", "user_username": self.user.username}, settings.JWT_SECRET_KEY, timedelta(minutes=1))
        payload = {"token": token}
        
        res = self.factory.post(reverse_lazy("users:user_verify"), data=payload, format="json")
        res_data = res.data
