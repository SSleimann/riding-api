...
class ClearExpiredRequestTravelsTestCase(TestCase):
    ...
    def test_clear_expired_request_travels(self):
        ...
        RequestTravel.objects.create(user=self.user, created_time=created_at, origin=origin, destination=dest)
        RequestTravel.objects.create(user=self.user, created_time=created_at, origin=origin, destination=dest)
        RequestTravel.objects.create(user=self.user, created_time=created_at, origin=origin, destination=dest)
        RequestTravel.objects.create(user=self.user, created_time=timezone.now(), origin=origin, destination=dest)
